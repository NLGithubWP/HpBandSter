...
# Every process has to look up the hostname
...
workers = []
for i in range(3):
    w = worker(run_id=args.run_id, host=host, nameserver=ns_host, nameserver_port=ns_port, timeout=220, id=i)
    w.run(background=True)
    workers.append(w)

# Let us load the old run now to use its results to warms-tart a new run with slightly
...
res = bohb.run(n_iterations=args.n_iterations, min_n_workers=3)
